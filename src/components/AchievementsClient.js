...
export default function AchievementsClient() {
  ...
  return (
    <section className="space-y-8">
      <div className="inline-flex rounded-2xl border border-white/10 bg-white/5 p-2">
        {tabs.map((tab) => (
          <button
            key={tab.key}
            type="button"
      ...
      <div className="relative border-l border-white/15 pl-8 space-y-8">
        {items.map((item) => (
          <RevealOnScroll key={item.title}>
            <article className="relative rounded-2xl border border-white/10 bg-slate-900/70 p-5 transition hover:border-cyan-300/40">
